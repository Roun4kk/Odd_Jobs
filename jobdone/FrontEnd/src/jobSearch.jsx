...
function JobSearch() {
  ...
  const LoadingSpinner = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-4">
      ...
      <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Searching...</p>
    </div>
  );
  ...
  const renderUserCard = (person) => {
    ...
    return (
      <div
        ...
        className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 bg-white dark:bg-gray-800 cursor-pointer mb-4"
      >
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white truncate">{person.username}</h3>
              {person.verified?.email && person.verified?.phoneNumber && (
                <BadgeCheck className="h-5 w-5 text-teal-400" />
              )}
            </div>
            {person.userBio && <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 line-clamp-2">{person.userBio}</p>}
            <div className="flex flex-wrap gap-1 mb-2">
              {person.userSkills?.slice(0, 5).map((skill, i) => (
                <span key={i} className="inline-block px-2 py-1 text-xs bg-teal-50 dark:bg-teal-900/50 text-teal-700 dark:text-teal-300 rounded-full border border-teal-200 dark:border-teal-500/20">{skill}</span>
              ))}
            </div>
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-gray-600 dark:text-gray-400 ml-1">{average > 0 ? `(${average.toFixed(1)})` : "No ratings yet"}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-white dark:bg-gray-900 pb-20 px-4 pt-4">
        <div className="mb-6 bg-teal-50 dark:bg-gray-800 p-4 rounded-xl shadow-sm">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-10 py-3 border border-teal-200 dark:border-gray-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 bg-white dark:bg-gray-700 text-teal-900 dark:text-gray-100 text-sm"
            />
            {searchTerm && (
              <button
                onClick={clearSearch}
        ...
        {searchError && (
          <p className="text-red-500 dark:text-red-400 text-sm mb-4 text-center bg-red-50 dark:bg-red-900/20 p-2 rounded-lg">{searchError}</p>
        )}
          ...
          <div className="w-full">
            {feedLoading ? <LoadingSpinner /> : (
              filteredUsers.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">{searchTerm.trim() ? "No users found" : "Start typing to search by skill or username."}</p>
              )
            )}
          </div>
        )}
          ...
          <div className="w-[57%] flex flex-col items-center gap-6 pt-6">
            <div className="relative w-11/12">
              <input
                ...
                className="p-3 border w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-teal-500 dark:focus:border-teal-400 pr-10"
              />
              {searchTerm && (
                <button
                  onClick={clearSearch}
            ...
            {searchError && (
              <div className="w-11/12 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 rounded-md text-red-700 dark:text-red-400 text-sm">
                {searchError}
              </div>
            )}

            {searchTerm && !feedLoading && !searchError && (
              <div className="w-11/12 text-sm text-gray-600 dark:text-gray-400">
