      ...
      <div className="min-h-screen bg-white pb-20 px-4 pt-4">
        <div className="mb-4">
          <input
            ...
            className="w-full p-3 border border-gray-300 rounded-md focus:outline-teal-500"
          />
        </div>
        ...
        <div className="flex justify-around mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-full ${activeTab === "jobs" ? "bg-teal-500 text-white" : "bg-gray-100 text-gray-700"}`}
          >
            Jobs
          </button>
          <button
            onClick={() => setActiveTab("skills")}
            className={`px-4 py-2 rounded-full ${activeTab === "skills" ? "bg-teal-500 text-white" : "bg-gray-100 text-gray-700"}`}
          >
            Skills
          </button>
        </div>

        {searchError && <p className="text-red-500 text-sm mb-2 text-center">{searchError}</p>}
          ...
          <div className="w-full max-w-md mx-auto">
            {feedLoading ? <LoadingSpinner /> : (
              filteredUsers.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 text-center">{searchTerm.trim() ? "No users found" : "Start typing to search by skill or username."}</p>
              )
            )}
          </div>
        )}
